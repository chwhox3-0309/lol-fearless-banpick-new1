...
export default function LadderPage() {
  ...
  const assignLanes = useCallback((players: string[]): LaneAssignment[] => {
    ...
    // Assign shuffled players to fixed lanes in order
    for (let i = 0; i < shuffledPlayers.length; i++) {
      assignments.push({
        player: shuffledPlayers[i],
        lane: LANES[i], // Assign to fixed lane order
      });
    }
  ...
  const handleRandomStart = useCallback(() => {
    // Reset assignments before new calculation to clear previous results if any

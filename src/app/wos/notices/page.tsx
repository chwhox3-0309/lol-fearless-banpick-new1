...
export default function WosNoticesPage() {
  ...
  useEffect(() => {
    async function fetchNotices() {
      try {
        const res = await fetch('/api/wos/notices');
        if (!res.ok) {
  ...
  return (
    ...
    <div className="mb-4">
      <Link href="/wos" className="inline-block bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
        &larr; 뒤로가기
      </Link>
    </div>
      <h1 className="text-4xl font-bold mb-8 text-center">WOS 공지사항</h1>
